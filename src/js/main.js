...
let AllDisneyCharacters = [
  ...
  },
  {
    ...
    name: "Irwina Allen",
    imageUrl: "https://placehold.co/200x150?text=Disney",
...
// array de favoritos (hasta línea 1938)
let favourites = [
  {
    _id: 112,
    films: ["Hercules (film)"],
...
const handleFavourite = (ev) => {
  ...
  const selectedId = parseInt(ev.currentTarget._id);
  const clickedDisneyCharacterObj = AllDisneyCharacters.find(
    (eachCharacter) => eachCharacter._id === selectedId
  );
  const favouritesIdx = favourites.findIndex(
    (eachCharacter) => eachCharacter._id === selectedId
  );

  if ( favouritesIdx === -1 ) {
    favourites.push(clickedDisneyCharacterObj);
    renderFavourites();
  }
  else {
...
const renderOneDisneyCharacter = (DisneyCharacterObj) => {
  const html = `
    <li class = "DisneyItem js_DisneyItem" id="${DisneyCharacterObj._id}">
...
const renderFavourites = () => {
  ...
  const AllDisneyCharacterLi = document.querySelector(".DisneyItem");
  for (const li of favourites) {
...
fetch("https://api.disneyapi.dev/character?")
  ...
  .then((data) => {
    ...
    if (AllDisneyCharacters.length > 0) {
      renderAllDisneyCharacters();
    } else {
  ...
  if ( localStorage.getItem('DisneyCharactersFavs') !== null) {
    favourites = JSON.parse(localStorage.getItem('startshipsFavs'))
    renderFavourites();
  };
